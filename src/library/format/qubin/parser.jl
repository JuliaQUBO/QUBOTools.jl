function read_model(path::AbstractString, fmt::Format{:qubin})
    return HDF5.h5open(path, "r") do fp
        return read_model(fp, fmt)
    end
end

function read_model(fp::P, fmt::Format{:qubin}) where {P<:Union{HDF5.File,HDF5.Group}}
...
function read_solution(path::AbstractString, fmt::Format{:qubin})
    return HDF5.h5open(path, "r") do fp
        return read_solution(fp, fmt)
    end
end

function read_solution(fp::P, fmt::Format{:qubin}) where {P<:Union{HDF5.File,HDF5.Group}}
...
function _parse_model_form(fp::P, ::Format{:qubin}) where {P<:Union{HDF5.File,HDF5.Group}}
...
function _parse_model_variables(fp::P, ::Format{:qubin}) where {P<:Union{HDF5.File,HDF5.Group}}
...
function _parse_solution_data(fp::P, ::Format{:qubin}) where {P<:Union{HDF5.File,HDF5.Group}}
